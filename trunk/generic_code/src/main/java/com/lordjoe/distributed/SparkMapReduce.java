 ...
 * Date: 8/25/2014
 */
public class SparkMapReduce<VALUEIN extends Serializable, K extends Serializable, V extends Serializable> extends AbstractMapReduceEngine<VALUEIN, K, V> {
   ...
   protected void showOutput() {
    ...
    protected void handlePartition(Iterable<KeyValueObject<K, V>> partition) {
        ...
        List<IKeyValueConsumer<K, V>> consumers = getConsumers();
        IKeyValueConsumer<K, V> consumer = consumers.get(0);
        K key = null;
        List<V> holder = new ArrayList();
        for (KeyValueObject<K, V> kv : partition) {
            if (!kv.key.equals(key)) {
                if (!holder.isEmpty()) {
                    reduce.handleValues(key, holder, consumer);// todo this is values
                }
                holder.clear();
                key = kv.key;
            }
